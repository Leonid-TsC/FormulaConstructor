...
Функция СтрУникальныйИдентификатор(ИсточникИдентификатора) Экспорт
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ИсточникИдентификатора)) Тогда
		...
		СтрУникальныйИдентификатор = tsc_КомпоновкаДанныхСервер.УникальныйИдентификаторВСтроку(УникальныйИдентификатор);
...
Функция ПараметрыДиаграмм(КомпоновщикНастроек) Экспорт
	...
	ПараметрМаксимумСерий = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "МаксимумСерийКоличество");
	...
	ПараметрТипДиаграммы = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ТипДиаграммы");
	...
	ПараметрВыделениеСерийДиаграмм = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ВыделениеСерийДиаграмм");
	...
	ПараметрГрадиентСерийДиаграмм = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ГрадиентСерийДиаграмм");
	...
	ПараметрОтображениеМаркеровТочекДиаграмм = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ОтображениеМаркеровТочекДиаграмм");
	...
	ПараметрОтслеживаемыеАналитики = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ОтслеживаемыеАналитики");
...
Процедура ДобавитьПредставленияПараметраВМассив(СхемаКомпоновкиДанных, КомпоновщикНастроек, Параметры, ИмяПараметра) Экспорт
	...
	Если Не Параметр = Неопределено Тогда
		...
		ПараметрКомпоновки = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, ИмяПараметра);
...
Функция ЗаголовкиПолейЕдиницИзмерений(КомпоновщикНастроек, ПереопределениеЗаголовков = Неопределено) Экспорт
	...
	ПараметрЕдиницыКоличества = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "ЕдиницыКоличества");
...
Процедура НастроитьДинамическийПериод(СхемаКомпоновкиДанных, КомпоновщикНастроек, ДополнятьПериод = Ложь, ДополнениеПути = "") Экспорт
	...
	ПараметрПериодичность = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "Периодичность");
	
	Если ПараметрПериодичность <> Неопределено 
		И tsc_КомпоновкаДанныхКлиентСервер.ПолеИспользуется(КомпоновщикНастроек, Новый ПолеКомпоновкиДанных(ДополнениеПути + "ДинамическийПериод")) Тогда
		...
		Если ИскомоеПоле <> Неопределено Тогда
			...
			ВыражениеПериод = tsc_КомпоновкаДанныхКлиентСервер.СтрокиПериода(ПараметрПериодичность.Значение).Период;
			...
			Если ДополнятьПериод Тогда
				...
				ПараметрПериод = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек, "Период"); // ЗначениеПараметраНастроекКомпоновкиДанных, Неопределено -
				
				Если ПараметрПериод <> Неопределено Тогда
					
					ПолеДинамическийПериод = Новый ПолеКомпоновкиДанных(ДополнениеПути + "ДинамическийПериод");
					Группировки = tsc_КомпоновкаДанныхКлиентСервер.ПолучитьГруппировки(КомпоновщикНастроек);
...
Процедура ОптимизацияИспользованияОборотнойАналитики(МакетКомпоновкиДанных, СхемаКомпоновкиДанных, НастройкиОтчета, Знач ДанныеРасшифровки) Экспорт
	...
	tsc_КомпоновкаДанныхКлиентСервер.УстановитьПараметр(НастройкиОтчета, "ИспользуетсяОборотнаяАналитика", ИспользуетсяОборотнаяАналитика);
...
Функция СкопироватьНаборыДанных(ПриемникНаборовДанных, ИсточникНаборовДанных, ОсновноеИмяНабораДанных = "") Экспорт
	...
	Для Каждого НаборИсточник Из Источник Цикл 
		...
		НовыйНабор = tsc_КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(Приемник, ТипНабора, ИмяНабора);
...
Функция СкопироватьПоляНабораДанных(ПриемникПолейНабораДанных, ИсточникПолейНабораДанных, ДублироватьИмяВПутиПоля = Ложь) Экспорт
	...
	Для Каждого ПолеИсточника Из ПоляИсточника Цикл 
		...
		Если ТипЗнч(ПолеИсточника) = Тип("ПолеНабораДанныхСхемыКомпоновкиДанных") Тогда
			ПолеПриемника = tsc_КомпоновкаДанныхСервер.НовоеПолеНабора(ПриемникПолейНабораДанных, "НовоеПоле");
		...
		Иначе
			ПолеПриемника = tsc_КомпоновкаДанныхСервер.НоваяГруппаНабора(ПриемникПолейНабораДанных, "НоваяГруппа");
			ЗаполнитьЗначенияСвойств(ПолеПриемника, ПолеИсточника);
		КонецЕсли;
...
Процедура ДобавитьОтборПоКолонкеТаблицыЗначений(ЭлементНастроекКомпоновки, ТаблицаДанных, ИмяКолонки) Экспорт
	...
	Если Не (МассивЗначенийОтбора.Количество() = 1 
		...
		tsc_КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(ЭлементНастроекКомпоновки,
			ИмяКолонки,
			СписокЗначений,
...
Функция СтруктураСерийЦветов(СерииДиаграммы)
	...
	Для Каждого СерияДиаграммы Из СерииДиаграммы Цикл
		...
		tsc_КомпоновкаДанныхКлиентСервер.ДополнитьСтруктуру(СтруктураЦветаСерии, СтруктураЦвета, Истина);
		
		ВидЦветаСерии = СерияДиаграммы.Цвет.Вид;
		Если ВидЦветаСерии = ВидЦвета.WebЦвет Тогда
			tsc_КомпоновкаДанныхСервер.ПолучитьКрасныйЗеленыйСинийВебЦвета(СерияДиаграммы.Цвет,
	...
	Для Каждого СерияРазныхЦветов Из СтруктураСерийЦветов Цикл 
		...
		Для Каждого ОбрабатываемаяСерия Из ОбрабатываемыеСерии Цикл
			Если Тон + Насыщенность + Светлота = 0 Тогда
				tsc_КомпоновкаДанныхСервер.ПолучитьТонНасыщенностьЯркостьПоКрасныйЗеленыйСиний(ЦветСерии["Красный"],
					ЦветСерии["Зеленый"],
					ЦветСерии["Синий"],
					Тон,
					Насыщенность,
			...
			tsc_КомпоновкаДанныхСервер.ПолучитьКрасныйЗеленыйСинийПоТонуНасыщенностиЯркости(Тон, 
...
Функция СхемаКомпоновкиПоПараметру(Знач СхемаКомпоновки) Экспорт
	...
	Если ТипЗнч(СхемаКомпоновки) = Тип("Строка") Тогда
		...
		Иначе
			Возврат tsc_КомпоновкаДанныхСервер.МакетПоИмени(СхемаКомпоновки);
		КонецЕсли;
