...
Процедура ПолучитьТекстРасчета(ТекстРасчета, Операнды, ЗначениеЗамены, ТипыОперандов = Неопределено) Экспорт
	...
	Для Каждого Операнд Из Операнды Цикл
		...
		Если ЗначениеЗамены = "1 " Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ТекстРасчета = СтрЗаменить(ТекстРасчета, tsc_РаботаСФормуламиКлиентСервер.ПолучитьТекстОперандаДляВставки(Операнд), ЗначениеДляПодстановки);
	КонецЦикла;

КонецПроцедуры

Функция МассивПрефиковАлгоритмов() Экспорт
	Возврат tsc_РаботаСФормуламиПовтИсп.МассивПрефиковАлгоритмов();	
КонецФункции
#КонецОбласти
